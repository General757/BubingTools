...
/**
 * @ClassName: LoopLoadingRenderer
 * @Author: Bubing
 * @Date: 2020/8/24 3:47 PM
 * @Description: java类作用描述
 */
public class LoopLoadingRenderer extends LoadingRenderer {
    ...
    private static final float DEFAULT_CENTER_RADIUS = 7f;
    private static final float DEFAULT_STROKE_WIDTH = 2.5f;
    private static final float DEFAULT_PADDING = 7f;

    private static final int[] DEFAULT_COLORS = new int[]{Color.parseColor("#f7584f"), Color.parseColor("#f7584f"), Color.parseColor("#f7584f")};
    ...
    private LoopLoadingRenderer(Context context) {
        super(context);
        init(context);
        setupPaint();
    ...
    private void init(Context context) {
        ...
        mPadding = DensityUtils.dip2px(context, DEFAULT_PADDING);
    ...
    protected void draw(Canvas canvas) {
        ...
        mTempBounds.inset(mPadding, mPadding);

        mPaint.setColor(Color.WHITE);
        canvas.drawCircle(mTempBounds.left + (mTempBounds.right - mTempBounds.left) / 2, mTempBounds.top + (mTempBounds.bottom - mTempBounds.top) / 2, (mTempBounds.right - mTempBounds.left) / 2, mPaint);
