...
public class Result {
    ...
    public static int addPhoto(Photo photo) {
        if (Setting.videoCount != -1 || Setting.pictureCount != -1) {
            ...
            if (photo.type.contains(Constants.Type.VIDEO) && number >= Setting.videoCount) {
                return -2;
            }
            number = photos.size() - number;
            if ((!photo.type.contains(Constants.Type.VIDEO)) && number >= Setting.pictureCount) {
                return -1;
            }
        }
        photo.selected = true;
    ...
    private static int getVideoNumber() {
        int count = 0;
        for (Photo p : photos) {
            if (p.type.contains(Constants.Type.VIDEO)) {
                count += 1;
            }
        }
        return count;
